...
class AnalysisClass:
    ...
    self.required_ana_keys = []
    self.optional_ana_keys = []


    def __init__(self, data_dir, dataset, qubit_index, folder="study_data", expt_name="qspec_ge", ana_params={}):
    ...
    def load_all(self, verbose=False):
        ...
        for i,h5_file in enumerate(h5_files):
            load_data = load_h5_data(os.path.join(data_path, h5_file), 'QSpec')
            timestamps = get_data_field(load_data, 'QSpec', self.qubit_index, 'Dates')
    ...
    def run_analysis(self, verbose=False):
        ...
        for k in self.analysis_data.keys():
            print(self.analysis_data[k]["timestamps"][0])
