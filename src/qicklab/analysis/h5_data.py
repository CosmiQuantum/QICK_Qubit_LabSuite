        ...
        for q_key in range(number_of_qubits):
            ...
            if isinstance(I[0], float) or isinstance(Q[0], float):
                if 'T1' in decoherence_type:
                    decoherence_time, fit_err = t1_fit(I, Q, delay_times=delay_times)
                    if discard_values_over is not None:
                        if decoherence_time > discard_values_over:
                            continue

                decoherence_times[q_key].extend([decoherence_time])
                fit_errs[q_key].extend([fit_err])
                # Flatten the date list instead of nesting it.
                if isinstance(date, list):
                    date_times[q_key].extend(date)
                else:
                    date_times[q_key].append(date)
